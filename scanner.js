/**
 * Elite Confluence Futures Scanner v3.0
 * Dual-score direction lock | Soft BTC/ADX regime | Anti flip-flop
 */
...
async function getJson(url) {
  const res = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "EliteCryptoScanner/3.0" },
  });
...
function scoreSignal(h1, m15, m5, funding, btcBias) {
  ...
  const adx1h = h1.adx != null ? h1.adx : 0;
  const adx15 = m15.adx != null ? m15.adx : 0;
  const adxMax = Math.max(adx1h, adx15);
  const isChop = adxMax < 16;
  const isTrendStrong = adxMax >= 25;
  const vol = m5.volume;
  const volSupportLong = vol.pressure > 6 || (vol.spike && vol.pressure > 0);
  const volSupportShort = vol.pressure < -6 || (vol.spike && vol.pressure < 0);

  // Dual independent scoring — LONG and SHORT paths scored separately
  let longS = 0;
  let shortS = 0;
  ...
  if (h1Bull) longS += 18;
  if (h1Bear) shortS += 18;
  if (h1.emaBull) longS += 8;
  if (h1.emaBear) shortS += 8;
  if (h1.ms.trend === "up") longS += 10;
  if (h1.ms.trend === "down") shortS += 10;
  longS += Math.max(0, h1.biasScore) * 0.12;
  shortS += Math.max(0, -h1.biasScore) * 0.12;

  // 15m alignment
  if (m15Bull) longS += 10;
  if (m15Bear) shortS += 10;
  if (m15.emaBull) longS += 5;
  if (m15.emaBear) shortS += 5;
  longS += Math.max(0, m15.biasScore) * 0.08;
  shortS += Math.max(0, -m15.biasScore) * 0.08;

  // 5m trigger / mean-rev / squeeze
  if (m5.meanLong) longS += 16;
  if (m5.meanShort) shortS += 16;
  if (m5.macdUp) longS += 6;
  if (m5.macdDown) shortS += 6;
  if (m5.reversal.bias === "bullish") longS += 8 * m5.reversal.quality;
  if (m5.reversal.bias === "bearish") shortS += 8 * m5.reversal.quality;
  if (m5.emaBull) longS += 4;
  if (m5.emaBear) shortS += 4;
  longS += Math.max(0, m5.biasScore) * 0.06;
  shortS += Math.max(0, -m5.biasScore) * 0.06;

  if (m5.squeeze || m15.squeeze) {
    if (vol.pressure >= 10 && m5.macdUp) longS += 12;
    if (vol.pressure <= -10 && m5.macdDown) shortS += 12;
    if (m5.squeeze) { longS += 2; shortS += 2; }
  }

  // Volume confirmation
  if (volSupportLong) longS += 9;
  if (volSupportShort) shortS += 9;
  if (vol.spike) {
    if (vol.pressure > 0) longS += 4;
    if (vol.pressure < 0) shortS += 4;
  }

  // RSI context
  if (m5.rsi < 35) longS += 6;
  if (m5.rsi < 28) longS += 4;
  if (m5.rsi > 65) shortS += 6;
  if (m5.rsi > 72) shortS += 4;
  if (m5.rsi > 75) longS -= 10;
  if (m5.rsi < 25) shortS -= 10;

  // Funding
  if (funding < -0.00025) longS += 4;
  if (funding > 0.00025) shortS += 4;

  // ADX soft regime — never hard-kill mean reversion
  if (isTrendStrong) {
    if (h1Bull) longS += 5;
    if (h1Bear) shortS += 5;
  } else if (isChop) {
    if (!m5.meanLong) longS -= 8;
    if (!m5.meanShort) shortS -= 8;
    if (m5.meanLong) longS += 4;
    if (m5.meanShort) shortS += 4;
  }

  // BTC bias SOFT only. Never veto.
  if (btcBias && Math.abs(btcBias.score) >= 20) {
    if (btcBias.bias === "bullish") { longS += 6; shortS -= 4; }
    else if (btcBias.bias === "bearish") { shortS += 6; longS -= 4; }
  }

  // Anti-conflict penalties (not veto)
  if (h1Bear && longS > 0) longS -= 12;
  if (h1Bull && shortS > 0) shortS -= 12;
  if (m5.bias === "bearish" && m5.biasScore < -35) longS -= 10;
  if (m5.bias === "bullish" && m5.biasScore > 35) shortS -= 10;

  longS = clamp(Math.round(longS), 0, 99);
  shortS = clamp(Math.round(shortS), 0, 99);

  // Clear winner only — anti flip-flop
  const MARGIN = 10;
  let action = null;
  let conf = 0;
  if (longS >= shortS + MARGIN && longS >= 55) {
    action = "LONG";
    conf = longS;
  } else if (shortS >= longS + MARGIN && shortS >= 55) {
    action = "SHORT";
    conf = shortS;
  } else {
    return null;
  ...
  const isSq =
    ...
    ((action === "LONG" && vol.pressure >= 8) || (action === "SHORT" && vol.pressure <= -8));
  ...
  if (action === "LONG" && vol.pressure < -18) return null;
  if (action === "SHORT" && vol.pressure > 18) return null;

  if (isTrendAligned) conf = Math.min(99, conf + 5);
  if (isMean && ((action === "LONG" && m5.rsi < 40) || (action === "SHORT" && m5.rsi > 60))) {
    conf = Math.min(99, conf + 4);
  }
  if (isSq) conf = Math.min(99, conf + 3);
  conf = clamp(Math.round(conf), 0, 99);

  let setup = "TREND";
  if (isMean) setup = "MEAN_REV";
  else if (isSq || m5.squeeze || m15.squeeze) setup = "SQUEEZE";
...
function formatTelegramMessage(s) {
  ...
  return (
    ...
    `<i>Elite Scanner v3 · Risk max 0.75% · Not financial advice</i>`
  );
}
...
async function sendDiscord(signals) {
  ...
  for (let i = 0; i < signals.length; i++) {
    ...
    const embed = {
      ...
      footer: { text: "Elite Scanner v3 · Risk max 0.75% · NFA" },
      timestamp: new Date().toISOString(),
    };
    const res = await fetch(DISCORD_WEBHOOK, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username: "Elite Scanner", embeds: [embed] }),
    });
...
async function main() {
  console.log("=== ELITE Scanner v3.0 | Dual-score confluence | Soft regime ===");
  ...
  console.log(`Elite signals (direction-locked): ${signals.length}`);
