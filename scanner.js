/**
 * Strict Quality Futures Scanner v2.3
 * Discord + Telegram + Square | ADX + BTC bias + strict squeeze (v2.3)
 */
...
async function getJson(url) {
  const res = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "StrictCryptoScanner/2.3" },
  });
...
function scoreSignal(h1, m15, m5, funding, btcBias) {
  ...
  if (!action && m5.squeeze && m5.volume.spike && Math.abs(m5.volume.pressure) >= 12) {
    if (m5.volume.pressure > 12 && m5.macdUp && !h1Bear) action = "LONG";
    if (m5.volume.pressure < -12 && m5.macdDown && !h1Bull) action = "SHORT";
  }
  if (!action) return null;

  const isMean = m5.meanLong || m5.meanShort;
  const isSq = m5.squeeze || m15.squeeze;
  if (!isMean && !isSq && isChop) return null;
  if (!isMean && !isSq && adxMax < 22 && !m5.volume.spike) return null;
  ...
  if (btcBias && btcBias.bias !== "neutral" && Math.abs(btcBias.score) >= 25) {
    if (action === "LONG" && btcBias.bias === "bearish") return null;
    if (action === "SHORT" && btcBias.bias === "bullish") return null;
  }
  ...
  if (adxMax >= 28) conf += 6;
  else if (adxMax >= 22) conf += 3;
  else if (adxMax < 18) conf -= 6;

  if (btcBias) {
    if (action === "LONG" && btcBias.bias === "bullish") conf += 4;
    if (action === "SHORT" && btcBias.bias === "bearish") conf += 4;
  }
  ...
  return { action, probability: conf, setup, h1, m15, m5, adx: adxMax };
}
...
async function main() {
  console.log("=== Strict Scanner v2.3 (ADX + BTC bias + tight squeeze) ===");
  ...
  try {
    ...
    if (btcTF) {
      ...
      console.log(`BTC bias: ${btcBias.bias} (score ${btcBias.score}, ADX ${btcBias.adx != null ? btcBias.adx.toFixed(1) : "—"})`);
    }
  } catch (e) {
