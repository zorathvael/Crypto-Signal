...
async function getJson(url) {
  const res = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "StrictCore/2.8.0" },
  });
...
function formatTelegramMessage(s) {
  ...
  return (
    ...
    `<i>Strict Core v2.8 · Regime + OB Quality · Risk max 0.75% · NFA</i>`
  );
}
...
function formatSquareBatchMessage(coins) {
  ...
  lines.push("");
  lines.push("Strict Core v2.8 · Regime + OB Quality · Risk max 0.75% · NFA");
  lines.push("");
  lines.push(fo);
  return lines.join("\n").trim();
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="${pad}" y="48" font-family="Arial, Helvetica, sans-serif" font-size="32" font-weight="700" fill="#ffffff">STRICT CORE v2.8</text>
...
async function sendDiscord(signals) {
  ...
  for (let i = 0; i < signals.length; i++) {
    ...
    const embed = {
      ...
      footer: { text: "Strict Core v2.8 · Regime + OB Quality · NFA" },
...
async function main() {
  console.log("=== Strict Core v2.8.0 | Regime + OB Quality + Risk ===");
  ...
  for (const c of candidates) {
    try {
      ...
      const riskPct = suggestRisk(regime, scored.probability);
